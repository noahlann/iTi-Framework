...
public class PkceAuthorizationCodeTokenGranter extends AuthorizationCodeTokenGranter {
    ...
    public PkceAuthorizationCodeTokenGranter(AuthorizationServerTokenServices tokenServices, AuthorizationCodeServices authorizationCodeServices, ClientDetailsService clientDetailsService, OAuth2RequestFactory requestFactory) {
        ...
        this.authorizationCodeServices = (PkceAuthorizationCodeServices) authorizationCodeServices;
    }
    ...
    protected OAuth2Authentication getOAuth2Authentication(ClientDetails client, TokenRequest tokenRequest) {
        ...
        // add pkce flow for special code_verifier
        if (parameters.containsKey(KEY_CODE_VERIFIER)) {
            storedAuth = authorizationCodeServices.consumeAuthorizationCode(authorizationCode);
        } else {
            String codeVerifier = parameters.getOrDefault(KEY_CODE_VERIFIER, "");
