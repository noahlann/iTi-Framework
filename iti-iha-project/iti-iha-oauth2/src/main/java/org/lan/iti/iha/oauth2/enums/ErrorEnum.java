...
@Getter
public enum ErrorEnum implements IEnum<String> {
    /**
     * Authorization error code
     */
    ...
    public static ErrorEnum getByError(String error) {
        if (StringUtil.isEmpty(error)) {
            return AUTHORIZATION_FAILED;
        }
        ErrorEnum[] errorRespons = ErrorEnum.values();
        for (ErrorEnum errorEnum : errorRespons) {
            if (errorEnum.getCode().equalsIgnoreCase(error)) {
                return errorEnum;
            }
        }
        return AUTHORIZATION_FAILED;
