...
public class TenantContextHolderFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        ...
        log.debug("请求Header中租户ID为：{}", tenantId);

        if (StrUtil.isNotBlank(tenantId) || StringUtils.isNullOrUndefined(tenantId)) {
            TenantContextHolder.setTenantId(ITIConstants.DEFAULT_TENANT_ID);
        } else {
