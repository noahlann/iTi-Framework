...
import org.lan.iti.common.ddd.step.IReviseStepsException;
import org.lan.iti.common.ddd.step.IRevokableDomainStep;
...
public abstract class AbstractStepsExecTemplate<Step extends IDomainStep, Model extends IDomain> {
    ...
    public final void execute(String activityCode, List<String> stepCodes, Model model) throws RuntimeException {
        execute(activityCode, stepCodes, model, null, EMPTY_ASYNC_STEPS);
    }
    ...
    private List<String> executeSteps(String activityCode, List<String> stepCodes, Stack<IRevokableDomainStep> executedSteps, Model model,
        ...
        if (asyncStepCodes == null || taskExecutor == null) {
            // the sentry
            asyncStepCodes = EMPTY_ASYNC_STEPS;
        }
        ...
        try {
            ...
            if (!executedSteps.empty()) {
                // Step必定是同一个ClassLoader加载的：中台统一加载
    ...
    private void asyncExecuteStep(SchedulingTaskExecutor taskExecutor, Step step, Model model) {
        ...
        taskExecutor.execute(() -> {
            ...
            try {
                // IMPORTANT: model must be thread safe!
                step.execute(model);
            } finally {
                MDC.clear();
            }
    ...
    private void safeRollbackExecutedSteps(Model model, RuntimeException cause, Stack<IRevokableDomainStep> executedSteps) {
        while (!executedSteps.isEmpty()) {
            ...
            try {
                ...
                log.error("step:{} rollback err ignored, model:{}", executedStep.stepCode(), model, t);
            }
        }
    }
}
